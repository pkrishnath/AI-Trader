...
def get_available_date_range(modelname: str) -> Tuple[str, str]:
    ...
    position_file = (
        base_dir / "data" / "agent_data" / modelname / "position" / "position.jsonl"
    )
...
def print_performance_report(metrics: Dict[str, any]) -> None:
    ...
    if metrics["max_drawdown_start"] and metrics["max_drawdown_end"]:
        print(
            f"  Drawdown Period: {metrics['max_drawdown_start']} to {metrics['max_drawdown_end']}"
        )
    print()
...
def save_metrics_to_jsonl(
    metrics: Dict[str, any], modelname: str, output_dir: Optional[str] = None
) -> str:
    """
...
def get_latest_metrics(
    modelname: str, output_dir: Optional[str] = None
...
def print_metrics_summary(modelname: str, output_dir: Optional[str] = None) -> None:
    ...
    print(
        f"   Analysis Period: {latest['analysis_period']['start_date']} to {latest['analysis_period']['end_date']}"
    )
