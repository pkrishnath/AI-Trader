...
- **Weekly Profile:** What is the trend over the last several weeks? Is price seeking higher highs (bullish) or
  lower lows (bearish)? Identify major weekly support and resistance levels or old highs/lows that could act as a
  magnet for price.
- **Daily Profile:** Zoom into the last several days. What is the immediate daily trend, and does it align with your
  weekly bias? Identify the "draw on liquidity" for the current day – where is price most likely to go today?
...
- Once you have a clear higher-timeframe bias, use the `Today's Intraday Prices` (e.g., 60-minute for stocks,
  30-minute for crypto) to find a precise entry that aligns with your bias.
- **Crucial Rule:** Only take trades in the direction of your established weekly/daily bias. If you are bullish,
  only look for long (buy) setups. If you are bearish, only look for short (sell) setups.
- Follow these steps to find an entry:
    a. **Wait for a Liquidity Grab:** Wait for the price to run above a recent high (if bearish) or below a recent
       low (if bullish). This is the "Judas Swing".
    b. **Look for a Market Structure Shift (MSS):** After the liquidity grab, watch for a strong reversal that
       breaks a recent swing point, signaling a change in direction.
    c. **Identify the Entry Point (FVG):** This reversal should create a "Fair Value Gap" (FVG) or "imbalance".
       This FVG is your entry zone.
    d. **Enter the Trade:** When the price retraces back into the FVG, execute your trade (buy or sell).
    e. **Set Profit Target:** Your target should be an opposing liquidity pool (e.g., if you buy, target a recent
       high where sell-side liquidity is resting).

**3. Explain Your Rationale**
- You must provide a detailed explanation for every trade, referencing the specific ICT concepts, price levels,
  and timeframes you analyzed.
- If you do not trade, explain why (e.g., "The intraday price action did not provide a valid entry setup that
  aligned with my bullish daily bias.").
...
class IctPromptGenerator:
    def __init__(self, asset_type: str, symbols: list):
        ...
        if self.asset_type == "crypto":
            ...
            raise NotImplementedError(
                f"Asset type '{self.asset_type}' is not yet supported in the IctPromptGenerator."
            )
    ...
    def _get_prices_string_toon(
        self, target_date: str = "", daily: bool = False
    ) -> str:
        prices_str = ""
        for symbol in self.symbols:
            if daily:
                ...
                prices_str += f"---\n{symbol} Intraday Prices for {target_date} ---\n"
    ...
    def generate_prompt(self, today_date: str, signature: str) -> str:
        print(
            f"Generating ICT prompt for {signature} on {today_date} (Asset: {self.asset_type})"
        )
        ...
        yesterday_intraday_toon = self._get_prices_string_toon(
