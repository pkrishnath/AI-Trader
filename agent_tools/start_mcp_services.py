...
class MCPServiceManager:
    ...
    def start_service(self, service_id, config):
        ...
        try:
            ...
            print(
                f"✅ {service_name} service started (PID: {process.pid}, Port: {port})"
            )
            return True
    ...
    def print_service_info(self):
        ...
        for service_id, service in self.services.items():
            print(
                f"  - {service['name']}: http://localhost:{service['port']} (PID: {service['process'].pid})"
            )
    ...
    def status(self):
        ...
        for service_id, config in self.service_configs.items():
            if service_id in self.services:
                ...
                if self.check_service_health(service_id):
                    print(
                        f"✅ {config['name']} service running normally (Port: {config['port']})"
                    )
                else:
                    print(
                        f"❌ {config['name']} service abnormal (Port: {config['port']})"
                    )
            else:
                print(
                    f"❌ {config['name']} service not started (Port: {config['port']})"
                )
